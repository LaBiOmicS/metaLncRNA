...
def generate_html_report(final_df, results_dict, sequence_stats, output_path):
    stats_df = pd.DataFrame.from_dict(sequence_stats, orient='index').reset_index().rename(columns={'index': 'sequence_id'})
    ...
    <body>
        ...
        <div class="grid">
            ...
            <div class="card" style="grid-column: span 2;">{fig_conf.to_html(full_html=False, include_plotlyjs=False)}</div>
        </div>
