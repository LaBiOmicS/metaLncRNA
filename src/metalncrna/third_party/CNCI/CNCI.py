...
def Tran_checkSeq (input_arr,Temp_Log):
    ...
    for i in range(len(label_Arr)):
        ...
        tran_sec_seq = tran_sec_seq_one.replace('\r','').replace('\n','')
        
        # Check for any character not in atcg
        if re.search(r'[^atcg]', tran_sec_seq):
            LogString = Label + ' contains non-canonical nucleotides, skipping...\n'
...
################################################################################################
    if ClassModel == 've':
        os.system('svm-scale -r '+ CNCIPATH + '/CNCI_Parameters/python_scale '+SvmoutPutFile+' > '+SvmPutFileName+'')
        os.system('svm-predict '+SvmPutFileName+' ' + CNCIPATH + '/CNCI_Parameters/python_model '+SvmFile+' > '+svm_tmp+'')
    if ClassModel == 'pl':
        os.system('svm-scale -r ' + CNCIPATH + '/CNCI_Parameters/plant_scale '+SvmoutPutFile+' > '+SvmPutFileName+'')
        os.system('svm-predict '+SvmPutFileName+' ' + CNCIPATH + '/CNCI_Parameters/plant_model '+SvmFile+' > '+svm_tmp+'')
##########################################################################################################
...
if Parallel > 1:
    ...
    for i in range(1,int(Parallel)+1):
        ...
        # Wait until file exists before checking length
        while not os.path.exists(Score_string) or not os.path.exists(Detil_string):
            time.sleep(0.5)
            
        SUB_SCORE_LEN,SUB_DETIL_LEN  = check(Score_string,Detil_string)
        # Added a small sleep and a safety break to the while loop
        retry_count = 0
        while (int(SUB_SCORE_LEN) < Con_ARRAY[n] or int(SUB_DETIL_LEN) < Con_ARRAY[n]) and retry_count < 100:
            time.sleep(1)
            SUB_SCORE_LEN,SUB_DETIL_LEN = check(Score_string,Detil_string)
            retry_count += 1
#######################################################################
