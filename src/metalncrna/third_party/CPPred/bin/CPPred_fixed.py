...
from feamodule import ORF_length as len_mod # renamed to avoid conflict
...
def output_feature(seq_file,hex_file,species):
    ...
    for seq in Seq.parse(seq_file,'fasta'):
        ...
        Len,Cov,inte_fe = len_mod.len_cov(seq.seq)
        ...
        # Use python 2 style print for compatibility with legacy env
...
def predict(range_file,model_file):
    # Use PATH-based svm tools
    os.system('svm-scale -r '+ range_file + ' test.f_svm  > test.scaled ')
    os.system('svm-predict -b 1 test.scaled ' + model_file +' tmp.txt >  tmp2.txt')
...
def deleted():
    os.system("rm -f test.*")
    os.system("rm -f coding_potential")
    os.system("rm -f tmp.txt tmp2.txt")

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('-i','--fasta',required=True, help="the input FASTA file")
    parser.add_argument('-hex','--hexamer',required=True, help="the input of hexamer")
    parser.add_argument('-r','--range',required=True, help="the input file of range")
    parser.add_argument('-m','--model',required=True, help="the input file of model")
    parser.add_argument('-s','--species',default="Human", help="the input species")
    parser.add_argument('-o','--outfile',required=True, help="output file")
    args = parser.parse_args()

    output_feature(args.fasta, args.hexamer, args.species)
