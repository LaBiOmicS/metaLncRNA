...
class Dispatcher:
    ...
    """
    def __init__(self, config: Dict[str, Any], n_jobs: Optional[int] = None, use_mamba: bool = True, keep_intermediates: bool = False):
        self.config = config
    ...
    def _init_adapters(self) -> Dict[str, Any]:
        adapters = {}
        def get_c(name): return self.config.get(name, {})
        
        if "rnasamba" in self.config:
            c = get_c("rnasamba")
            adapters["rnasamba"] = RNAsambaAdapter(weights_path=c.get("weights"), tool_path=c.get("path", "rnasamba"), env_name=c.get("env_name", "metalnc_rnasamba"))
        if "cpc2" in self.config:
            c = get_c("cpc2")
            adapters["cpc2"] = CPC2Adapter(tool_path=c.get("path", "CPC2.py"), env_name=c.get("env_name", "metalnc_cpc2"), use_mamba=self.use_mamba)
        if "cpat" in self.config:
            c = get_c("cpat")
            adapters["cpat"] = CPATAdapter(logit_model=c.get("logit_model"), hexamer_table=c.get("hexamer_table"), tool_name=c.get("path", "cpat.py"), env_name=c.get("env_name", "metalnc_cpat"))
        if "plek" in self.config:
            c = get_c("plek")
            adapters["plek"] = PLEKAdapter(tool_name=c.get("path", "PLEK.py"), env_name=c.get("env_name", "metalnc_plek"))
        
        # Internal / Legacy tools
        if "cppred" in self.config:
            c = get_c("cppred")
            adapters["cppred"] = CPPredAdapter(tool_name=c.get("path", "CPPred_fixed.py"), env_name=c.get("env_name", "metalnc_legacy"))
        if "cnci" in self.config:
            c = get_c("cnci")
            adapters["cnci"] = CNCIAdapter(mode=c.get("mode", "ve"), tool_name=c.get("path", "CNCI.py"), env_name=c.get("env_name", "metalnc_legacy"))
        if "lgc" in self.config:
            c = get_c("lgc")
            adapters["lgc"] = LGCAdapter(tool_name=c.get("path", "lgc-1.0.0.py"), env_name=c.get("env_name", "metalnc_legacy"))
